...
# Ici nous allons fixer la température T sur les bords ; ces valeurs ne seront pas modifiées par la suite, ce qui correspond # à une condition de température fixée et constante sur les frontières (contact avec un thermostat)
# bord gauche T[:,0] = np.zeros((NY)) # bord haut T[-1,:] = np.zeros((1,NX)) # bord droit T[:,-1] = np.zeros((NY)) # bord bas T[0,:] = np.zeros((1,NX))
...
# bord bas
jQ[0,:] = np.ones((1,NX))
# On peut ici éventuellement rajouter une zone "source" (siège de réactions exo# ou endo-thermiques
...
# frontière. # Si la ligne n'est pas commentée, c'est le flux thermique qui est fixé
